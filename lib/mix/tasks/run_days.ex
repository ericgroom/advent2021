defmodule Mix.Tasks.RunDays do
  ...
  def run(args) do
    {:ok, _} = :application.ensure_all_started(:advent2021)

    Advent2021.Days.all_days()
    |> run_days(args)
  end
  ...
  defp run_day(module, []) do
    if Kernel.function_exported?(module, :part_one, 1), do: IO.inspect(module.part_one())
    if Kernel.function_exported?(module, :part_two, 1), do: IO.inspect(module.part_two())
  end
